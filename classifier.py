...
class Classifier:
    def __init__(self, rule_file: str = "rules.csv"):
        ...
        with open(rule_file, 'r') as csv_file:
            ...
            for i, row in enumerate(csv_reader):
                if i == 0:
                    continue
                categories_dict[row[1]].append(row[2])
    ...
    def classify(self, symptoms_scores: Dict[str, int]):
        ...
        for symptom, s_score in symptoms_scores.items():
            category = self.get_category_by_symptom(symptom)
            if category is None:
                continue
    ...
    def get_category_by_symptom(self, symptom: str):
        return next(filter(lambda c: symptom in c.symptoms, self.categories), None)
